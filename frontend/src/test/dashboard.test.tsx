...
describe("customizable dashboard", () => {
  ...
  it("renders a truthful command-center default with a primary surface and dedicated AI rail", async () => { const { container } = render(<MemoryRouter><DashboardPage /></MemoryRouter>); expect(screen.getByRole("heading", { name: /engineering priorities/i })).toBeVisible(); expect(await screen.findByText("Visible projects")).toBeVisible(); expect(container.querySelector('[data-default-composition="true"]')).toBeTruthy(); expect(container.querySelector('[data-widget-id="engineering-work"]')).toBeTruthy(); expect(container.querySelector('[data-widget-id="projects"]')).toBeTruthy(); expect(container.querySelector('[data-widget-id="assistant"]')).toBeTruthy(); expect(screen.getByRole("heading", { name: "Active Projects" })).toBeVisible(); expect(screen.getByText("No visible active projects")).toBeVisible(); expect(screen.getByText("No visible Capture context is ready.")).toBeVisible(); expect(screen.queryByText(/99|1,000/)).not.toBeInTheDocument(); });
  it("supports keyboard-accessible reorder, resize, hide, restore and reset", async () => { const user = userEvent.setup(); const { container } = render(<MemoryRouter><DashboardPage /></MemoryRouter>); await user.click(screen.getByRole("button", { name: "Customize Dashboard" })); await user.click(screen.getByRole("button", { name: "Move Active Projects later" })); expect(container.querySelector('[data-default-composition="false"]')).toBeTruthy(); await user.selectOptions(screen.getByRole("combobox", { name: "Size of Active Projects" }), "wide"); await user.click(screen.getByRole("button", { name: "Hide Active Projects" })); expect(screen.queryByRole("heading", { name: "Active Projects" })).not.toBeInTheDocument(); await user.click(screen.getByRole("button", { name: "Active Projects" })); expect(screen.getByRole("heading", { name: "Active Projects" })).toBeVisible(); expect(localStorage.getItem(LAYOUT_KEY)).toBeTruthy(); await user.click(screen.getByRole("button", { name: /reset satco default/i })); expect(localStorage.getItem(LAYOUT_KEY)).toBeNull(); expect(container.querySelector('[data-default-composition="true"]')).toBeTruthy(); });
