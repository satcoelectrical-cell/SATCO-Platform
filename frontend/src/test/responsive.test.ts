...
describe("responsive and accessibility-critical styling", () => {
  ...
  it("preserves visible focus, reduced motion and contained narrow navigation", () => { expect(css).toContain(":focus-visible"); expect(css).toContain("prefers-reduced-motion"); expect(css).toContain("translateX(-102%)"); expect(css).toContain("visibility:hidden"); expect(css).toContain("pointer-events:none"); });
  it("mirrors the drawer while isolating engineering notation in RTL", () => { expect(css).toContain("inset-inline-start:0"); expect(css).toContain(":dir(rtl)"); expect(css).toContain("translateX(102%)"); expect(css).toContain("direction:ltr"); expect(css).toContain("unicode-bidi:isolate"); });
