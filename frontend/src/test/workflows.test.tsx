...
const { apiMock } = vi.hoisted(() => ({ apiMock: { customers: vi.fn(), createCustomer: vi.fn(), createProject: vi.fn(), updateProject: vi.fn(), projects: vi.fn(), project: vi.fn(), projectFoundation: vi.fn(), putProjectFoundation: vi.fn(), createProjectInput: vi.fn(), updateProjectInput: vi.fn(), reorderProjectInputs: vi.fn(), transitionProjectInput: vi.fn(), transitionProjectStage: vi.fn(), projectInputSources: vi.fn(), executionPlan: vi.fn(), workspaces: vi.fn(), createWorkspace: vi.fn(), captures: vi.fn(), createCapture: vi.fn(), reports: vi.fn(), reportSources: vi.fn(), reportEvidenceSources: vi.fn(), report: vi.fn(), createReport: vi.fn(), reviseReport: vi.fn(), acceptReport: vi.fn(), admitMemory: vi.fn(), memory: vi.fn(), memoryDetail: vi.fn(), supportingFiles: vi.fn(), evidence: vi.fn(), uploadSupportingFile: vi.fn(), linkSupportingFiles: vi.fn(), downloadSupportingFile: vi.fn() } }));
...
beforeEach(() => { for (const fn of Object.values(apiMock)) fn.mockReset(); apiMock.customers.mockResolvedValue({ state: "success", data: { items: [], total: 0, page: 1, size: 100 } }); apiMock.projectFoundation.mockResolvedValue({ state: "success", data: { outcome: "success", availability: "basis_not_established", project_id: 7, allowed_actions: ["establish"] } }); apiMock.executionPlan.mockResolvedValue({ state: "success", data: { outcome: "success", availability: "plan_not_established", project_id: 7, allowed_actions: [] } }); apiMock.supportingFiles.mockResolvedValue({ state: "success", data: { items: [], visible_count: 0, continuation: null } }); apiMock.evidence.mockResolvedValue({ state: "success", data: { items: [], total: 0, page: 1, size: 100 } }); apiMock.reportEvidenceSources.mockResolvedValue({ state: "success", data: { items: [], total: 0, page: 1, size: 20 } }); });
