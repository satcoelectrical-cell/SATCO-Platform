...
const { apiMock } = vi.hoisted(() => ({ apiMock: { standardsIntelligence: vi.fn() } }));
...
it("preserves deterministic evidence and labels provider suggestions advisory-only", async () => {
  ...
  await user.type(screen.getByLabelText(/Authorized snapshot IDs/), "snap-2, snap-1");
  await user.type(screen.getByLabelText(/Human-verified assertion IDs/), "assert-1");
...
it("shows a closed not-permitted outcome without unsafe advisory detail", async () => {
  ...
  const user = userEvent.setup(); render(<StandardsIntelligencePanel projectId={7} />);
  await user.type(screen.getByLabelText("Human advisory purpose"), "Check rights"); await user.type(screen.getByLabelText(/Authorized snapshot IDs/), "snap-1"); await user.click(screen.getByRole("button", { name: "Run advisory check" }));
...
it("clears stale advisory data and moves focus to a closed conflict status", async () => {
  ...
  const user = userEvent.setup(); render(<StandardsIntelligencePanel projectId={7} />);
  await user.type(screen.getByLabelText("Human advisory purpose"), "Check coordination gaps");
  await user.type(screen.getByLabelText(/Authorized snapshot IDs/), "snap-1");
  await user.click(screen.getByRole("button", { name: "Run advisory check" }));
