...
const { apiMock } = vi.hoisted(() => ({ apiMock: { projectStandards: vi.fn(), projectStandardCandidates: vi.fn(), declareProjectStandard: vi.fn(), retireProjectStandard: vi.fn(), retrieveStandardSources: vi.fn(), displayStandardSource: vi.fn(), createStandardAssertion: vi.fn(), decideStandardAssertion: vi.fn() } }));
...
it("keeps package candidates advisory and records an explicit Human applicability declaration", async () => {
  ...
  await user.type(screen.getByLabelText("Registry edition ID"), "ed-1");
