...
export function EngineeringPerformancePanel({projectId,workspaceId}:{projectId:number;workspaceId?:number|null}){
 ...
 async function openAction(action:Action){
  const sourceHandle=action.source_handles.find(handle=>handle.startsWith("indicator:"));
  setRouteMessage("");
  if(!sourceHandle){setRouteMessage("This advisory action has no supported current owner route.");return;}
  const path=await resolveOwnerRoute({routeKind:"engineering_performance_source",label:action.title_code.replaceAll("_"," "),projectId,workspaceId,targetHandle:sourceHandle});
  if(path)navigate(path);else setRouteMessage("The advisory source is stale or no longer available in this authorized context.");
 }
 ...
 return <section className="engineering-performance" aria-labelledby="engineering-performance-title" dir="auto"><Surface title="Engineering Performance" subtitle="Derived, advisory and non-authoritative">
  ...
  {!a?null:<section aria-label="Advisory next actions"><h3>Advisory next actions</h3>{a.actions.length?a.actions.map((x:Action)=><article key={x.action_key}><strong>{x.title_code.replaceAll("_"," ")}</strong><span>{x.status.replaceAll("_"," ")}</span><p>{x.rationale_codes.join(" · ").replaceAll("_"," ")}</p><small>{x.source_handles.join(" · ")}</small>{x.limitations.map(code=><small key={code}>Limitation: {code.replaceAll("_"," ")}</small>)}<button type="button" className="button ghost compact" onClick={()=>void openAction(x)}>Open current owner context</button></article>):<p>No advisory next actions are currently projected.</p>}{routeMessage?<p role="status">{routeMessage}</p>:null}</section>}
