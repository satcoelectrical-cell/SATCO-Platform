...
import type { EffectiveDisciplinePackages, OrganizationPackageConfiguration, PackageObjectCreateResult, PackageRuleResult, ProjectPackageConfiguration, SupportedPackages, WorkspacePackageApplicability } from "./types";
...
export const api = {
  crossDisciplineDefinitions:(projectId:number)=>request<{items:{definition_set_id:string;version:string;digest:string;supported_combinations:string[]}[];next_cursor:null}>(`/projects/${projectId}/cross-discipline/definitions`),
  crossDisciplineReadiness:(projectId:number)=>request<import("./types").XDIReadiness>(`/projects/${projectId}/cross-discipline/runtime-readiness`),
  crossDisciplineEligibility:(projectId:number,scope:import("./types").XDIScope)=>request<import("./types").XDIEligibility>(`/projects/${projectId}/cross-discipline/eligibility`,{method:"POST",body:JSON.stringify({scope})}),
  createCrossDisciplineAssessment:(projectId:number,payload:Record<string,unknown>)=>request<import("./types").XDIClosedResult>(`/projects/${projectId}/cross-discipline/assessments`,{method:"POST",body:JSON.stringify(payload)}),
  crossDisciplineAssessments:(projectId:number,cursor?:string)=>request<import("./types").XDIPage<import("./types").XDIAssessment>>(`/projects/${projectId}/cross-discipline/assessments${cursor?`?cursor=${encodeURIComponent(cursor)}`:""}`),
  crossDisciplineAssessment:(projectId:number,assessmentId:string)=>request<import("./types").XDIAssessment>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}`),
  crossDisciplineFindings:(projectId:number,assessmentId:string,cursor?:string)=>request<import("./types").XDIPage<import("./types").XDIFinding>>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/findings${cursor?`?cursor=${encodeURIComponent(cursor)}`:""}`),
  crossDisciplineFinding:(projectId:number,assessmentId:string,findingId:string)=>request<import("./types").XDIFinding>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/findings/${encodeURIComponent(findingId)}`),
  appendCrossDisciplineDisposition:(projectId:number,assessmentId:string,findingId:string,payload:Record<string,unknown>)=>request<import("./types").XDIDisposition>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/findings/${encodeURIComponent(findingId)}/dispositions`,{method:"POST",body:JSON.stringify(payload)}),
  crossDisciplineDispositions:(projectId:number,assessmentId:string,findingId:string)=>request<import("./types").XDIPage<import("./types").XDIDisposition>>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/findings/${encodeURIComponent(findingId)}/dispositions`),
  crossDisciplineLineage:(projectId:number,assessmentId:string)=>request<import("./types").XDIPage<import("./types").XDILineage>>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/lineage`),
  verifyCrossDisciplineAssessment:(projectId:number,assessmentId:string,payload:Record<string,unknown>)=>request<Record<string,unknown>>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/historical-verification`,{method:"POST",body:JSON.stringify(payload)}),
  createCrossDisciplineReassessment:(projectId:number,assessmentId:string,payload:Record<string,unknown>)=>request<import("./types").XDIClosedResult>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/reassessments`,{method:"POST",body:JSON.stringify(payload)}),
  supersedeCrossDisciplineAssessment:(projectId:number,assessmentId:string,payload:Record<string,unknown>)=>request<import("./types").XDIClosedResult>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/supersessions`,{method:"POST",body:JSON.stringify(payload)}),
  crossDisciplineDependency:(projectId:number,assessmentId:string,findingId:string)=>request<Record<string,unknown>>(`/projects/${projectId}/cross-discipline/assessments/${encodeURIComponent(assessmentId)}/findings/${encodeURIComponent(findingId)}/dependency-explanation`),
  ...
  replaceProjectPackageConfiguration: (id:number,payload:{expected_configuration_version:number;profile_id:string;selections:{package_key:string;package_version:string}[];rationale:string}) => request<ProjectPackageConfiguration>(`/projects/${id}/discipline-package-configuration`,{method:"PUT",headers:{"X-Correlation-ID":crypto.randomUUID()},body:JSON.stringify(payload)}),
  removeProjectPackageConfiguration: (id:number,payload:{expected_configuration_version:number;rationale:string}) => request<ProjectPackageConfiguration>(`/projects/${id}/discipline-package-configuration`,{method:"DELETE",headers:{"X-Correlation-ID":crypto.randomUUID()},body:JSON.stringify(payload)}),
  preflightProjectPackageConfiguration: (id:number,payload:{profile_id:string;selections:{package_key:string;package_version:string}[]}) => request<{decision:string;reason_codes:string[]}>(`/projects/${id}/discipline-package-configuration/preflight`,{method:"POST",body:JSON.stringify(payload)}),
  effectiveDisciplinePackages: (id:number) => request<EffectiveDisciplinePackages>(`/projects/${id}/effective-discipline-packages`),
  workspacePackageApplicability: (id:number) => request<WorkspacePackageApplicability>(`/workspaces/${id}/package-applicability`),
  createElectricalObject: (projectId:number, workspaceId:number, payload:{declaration_id:string;primary_identifier_display_value:string;primary_identifier_evidence_references:string[];steward_id:number|null;rationale:string}) => request<PackageObjectCreateResult>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/operations/objects`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID(),"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify(payload)}),
  evaluateElectricalRule: (projectId:number, workspaceId:number, payload:{hook_id:string;hook_version:"1.0.0";envelope:Record<string,unknown>}) => request<PackageRuleResult>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/rule-evaluations`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID()},body:JSON.stringify(payload)}),
  createInstrumentationObject: (projectId:number, workspaceId:number, payload:{declaration_id:string;primary_identifier_display_value:string;primary_identifier_evidence_references:string[];steward_id:number|null;rationale:string}) => request<PackageObjectCreateResult>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/operations/objects`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID(),"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify(payload)}),
  evaluateInstrumentationRule: (projectId:number, workspaceId:number, payload:{hook_id:string;hook_version:"1.0.0";envelope:Record<string,unknown>}) => request<PackageRuleResult>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/rule-evaluations`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID()},body:JSON.stringify(payload)}),
  createControlAutomationObject: (projectId:number, workspaceId:number, payload:{declaration_id:string;primary_identifier_display_value:string;primary_identifier_evidence_references:string[];steward_id:number|null;rationale:string}) => request<PackageObjectCreateResult>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/operations/objects`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID(),"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify(payload)}),
  evaluateControlAutomationRule: (projectId:number, workspaceId:number, payload:{hook_id:string;hook_version:"1.0.0";envelope:Record<string,unknown>}) => request<PackageRuleResult>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/rule-evaluations`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID()},body:JSON.stringify(payload)}),
  createControlAutomationRelationship: (projectId:number, workspaceId:number, payload:{declaration_id:string;source_object_id:string;target_object_id:string;evidence_references:string[];steward_id:number|null;rationale:string}) => request<Record<string,unknown>>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/operations/relationships`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID(),"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify(payload)}),
  bindControlAutomationContext: (projectId:number, workspaceId:number, payload:{input_declaration_id:string;context_id:number;context_subject_reference_id:number;expected_context_version:number;expected_configuration_revision:number;rationale:string}) => request<Record<string,unknown>>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/context-bindings`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID(),"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify(payload)}),
  bindControlAutomationEvidence: (projectId:number, workspaceId:number, payload:{input_declaration_id:string;evidence_id:string;expected_evidence_version:number;expected_configuration_revision:number;rationale:string}) => request<Record<string,unknown>>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/evidence-bindings`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID(),"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify(payload)}),
  createControlAutomationDeliverable: (projectId:number, workspaceId:number, payload:{declaration_id:string;code:string;title:string;discipline:"industrial_automation";deliverable_type:string;external_authority:"cad"|"document"|"spreadsheet";workspace_id:number;initial_external_label:string;rationale:string}) => request<DeliverableMutation>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/operations/deliverables`,{method:"POST",headers:{"X-Correlation-ID":crypto.randomUUID(),"Idempotency-Key":crypto.randomUUID()},body:JSON.stringify(payload)}),
  controlAutomationReadiness: (projectId:number, workspaceId:number, deliverableId:string, revisionId:string, configurationRevision:number, objectIds:string[], evidenceIds:string[]) => request<Record<string,unknown>>(`/projects/${projectId}/discipline-packages/workspaces/${workspaceId}/deliverables/${encodeURIComponent(deliverableId)}/revisions/${encodeURIComponent(revisionId)}/readiness?${new URLSearchParams([["expected_configuration_revision",String(configurationRevision)],...objectIds.sort().map(id=>["object_ids",id]),...evidenceIds.sort().map(id=>["evidence_ids",id])]).toString()}`,{headers:{"X-Correlation-ID":crypto.randomUUID()}}),
  ...
  createWorkspace: (projectId: number, payload: { discipline: string; description?: string | null }) => request<Workspace>(`/projects/${projectId}/workspaces`, { method: "POST", headers: { "X-Correlation-ID": crypto.randomUUID() }, body: JSON.stringify(payload) }),
