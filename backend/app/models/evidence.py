...
class Evidence(Base):
    ...
    def transition_lifecycle(self,c:TransitionEvidenceLifecycle,now:datetime):
        ...
        prev=self.version; self.lifecycle=target.value; self.replacement_evidence_id=c.replacement_evidence_id if target is EvidenceLifecycle.SUPERSEDED else None; self.version+=1; self.updated_at=now; e=self._event(c,"EvidenceLifecycleTransitioned",now); return EvidenceResult(self.id,prev,self.version,type(c).__name__,c.metadata.correlation_id,(e,))
