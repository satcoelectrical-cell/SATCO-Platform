...
from app.models.evidence_command import CreateEvidence, EvidenceCommandError, EvidenceMetadata, EvidenceVersionMismatch, TransitionEvidenceLifecycle
...
def _response(item,actions=()):
    data={c.name:getattr(item,c.name) for c in item.__table__.columns}; data["allowed_actions"]=actions
